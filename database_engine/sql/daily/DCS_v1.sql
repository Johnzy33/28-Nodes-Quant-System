...
Joined_Metrics AS (
    -- NOTE: Requires D_M1_Alignment_Prob to be present in asset_daily_conditional_metrics
    SELECT
        dc.trading_date, dc.asset_id,
        cm.D_M1_Bullish_Prob, cm.D_M1_Bearish_Prob, cm.D_M1_Alignment_Prob, -- <<< NEW COLUMN
...
Combined_D_M_Scores AS (
    ...
    SELECT
        trading_date, asset_id,
        -- REVISED D.M1 Score: Apply Temporal Alignment Multiplier
        ((D_M1_Bullish_Prob - D_M1_Bearish_Prob) * (1.0 + D_M1_Alignment_Prob)) AS D_M1_Score, 
        
        -- REVISED D.M2 Score: Weighted Structural Conviction (60% Week / 40% Day)
...
Factor_Calculation AS (
    -- 5. Convert probabilities into multiplicative factors (Final Boosted Formulas)
    SELECT
