...
Joined_Metrics AS (
    SELECT
        ...
        cm.D_M1_Bullish_Prob, cm.D_M1_Bearish_Prob, cm.D_M1_Alignment_Prob, 
        cm.D_M2_PDH_Prob, cm.D_M2_PDL_Prob, cm.D_M2_PWH_Prob, cm.D_M2_PWL_Prob, 
...
Combined_D_M_Scores AS (
    -- 3. Calculate D.M1 and D.M2 scores (Including Temporal Multiplier and Weighted D.M2)
    SELECT
        trading_date, asset_id,
        -- D.M1 Score: Apply Temporal Alignment Multiplier
        ((D_M1_Bullish_Prob - D_M1_Bearish_Prob) * (1.0 + D_M1_Alignment_Prob)) AS D_M1_Score, 
        -- D.M2 Score: Weighted Structural Conviction (60% Week / 40% Day)
        ((D_M2_PWH_Prob - D_M2_PWL_Prob) * 0.60 + (D_M2_PDH_Prob - D_M2_PDL_Prob) * 0.40) AS D_M2_Score, 
...
Base_Score AS (
    -- 4. Calculate the Daily Base Score (D.B.S.) using the REVISED WEIGHTING: 40% D.M1 / 60% D.M2
    SELECT
        trading_date, asset_id,
        (D_M1_Score * 0.40 + D_M2_Score * 0.60) AS DBS_Score, -- <<< REVISED WEIGHTING HERE
        d_m3_continuation_prob, d_m4_bullish_ft_prob, d_m4_bearish_ft_prob,
        d_m5_bullish_reversal_risk, d_m5_bearish_reversal_risk
    FROM Combined_D_M_Scores
),
Factor_Calculation AS (
    -- 5. Convert probabilities into multiplicative factors (Boosted Formulas)
    SELECT
