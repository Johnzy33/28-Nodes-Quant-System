...
-- Daily Table
--------
DROP TABLE IF EXISTS asset_daily_views;
CREATE TABLE asset_daily_views (
...
TRUNCATE asset_daily_views;

-- NOTE: This CTE assumes the existence of the following custom functions:
-- custom_session_group(time), get_trading_date(time), 
-- get_market_type(open, high, low, close) -> returns (session_type, consolidation_subtype),
-- get_close_type(open, high, low, close) -> returns TEXT ('Bullish_Close'/'Bearish_Close'),
-- calculate_nyam_bias(trading_date, asset_id) -> returns TEXT ('Bullish'/'Bearish'/'Neutral')
...
WITH base AS (
  ...
  FROM asset_market_data -- Assumes the source data table
),
perday AS (
