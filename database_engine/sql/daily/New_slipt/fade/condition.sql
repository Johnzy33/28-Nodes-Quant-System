...
CREATE TABLE asset_daily_composite_score (
    ...
    DCS NUMERIC(10, 4),             
    DCS_Classification TEXT,        
    -- 🔥 NEW: Contrarian Signal Columns 🔥
    DCS_Contrarian NUMERIC(10, 4),
    DCS_Contrarian_Classification TEXT, 
...
DCS_Calculation AS (
    SELECT
        ...
        -- Standard DCS (Structural Momentum)
        (DBS_Score_Raw * (1.0 + D_M1_Alignment_Prob) * CASE WHEN DBS_Score_Raw >= 0 THEN (1.0 - 0.2 * D_M5_Bullish_Reversal_Risk) ELSE (1.0 - 0.2 * D_M5_Bearish_Reversal_Risk) END) AS DCS_Final_Score,
        -- 🔥 2. Contrarian DCS Calculation 🔥
        (
            -- Flip the sign of DBS
            (DBS_Score_Raw * -1.0) 
            -- Multiply by the specific reversal risk for magnitude
            * CASE
                WHEN DBS_Score_Raw >= 0 THEN D_M5_Bullish_Reversal_Risk -- If strong Long momentum, FADE signal is magnitude of Bullish Risk
                ELSE D_M5_Bearish_Reversal_Risk                          -- If strong Short momentum, FADE signal is magnitude of Bearish Risk
            END
        ) AS DCS_Contrarian_Score,
        -- Placeholders for Insertion Logging
...
INSERT INTO asset_daily_composite_score (
    ...
    DCS_Contrarian, DCS_Contrarian_Classification -- <<< INSERT NEW COLUMNS
)
SELECT
    ...
    CASE
        WHEN d.DCS_Contrarian_Score >= 0.50 THEN 'High Conviction FADE Long' -- FADE is the Short/Bearish signal
        WHEN d.DCS_Contrarian_Score > 0.25 THEN 'Medium Conviction FADE Long'
        WHEN d.DCS_Contrarian_Score < -0.50 THEN 'High Conviction FADE Short' -- FADE is the Long/Bullish signal
