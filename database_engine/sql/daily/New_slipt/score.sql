...
Combined_D_M_Scores AS (
    SELECT trading_date, asset_id,
        -- 🔥 REVISED D.M1 Score: Recency Edge + Alignment Multiplier 🔥
        (
            -- ST Directional Edge: (B_ST - A_ST) - (B_LT - A_LT)
            (
                (D_M1_Bullish_Prob_ST - D_M1_Bearish_Prob_ST) 
                - (D_M1_Bullish_Prob_LT - D_M1_Bearish_Prob_LT)
            )
            * (1.0 + D_M1_Alignment_Prob) -- Alignment Multiplier
        ) AS D_M1_Score, 
        -- 🔥 D.M2 Score: ST vs LT Recency Edge Formula (Difference in Weighted Conviction) 🔥
        (
            -- ST Conviction - LT Conviction
            (
...
Base_Score AS (
    -- 4. Calculate the Daily Base Score (D.B.S.) using the 40% D.M1 / 60% D.M2 weighting
    SELECT trading_date, asset_id, (D_M1_Score * 0.40 + D_M2_Score * 0.60) AS DBS_Score, 
        d_m3_continuation_prob, d_m4_bullish_ft_prob, d_m4_bearish_ft_prob,
        d_m5_bullish_reversal_risk, d_m5_bearish_reversal_risk
...
Factor_Calculation AS (
    ...
    SELECT trading_date, asset_id, DBS_Score,
        (0.5 + 1.0 * d_m3_continuation_prob) AS F_Commitment, 
        CASE WHEN DBS_Score >= 0 THEN (0.7 + 1.8 * d_m4_bullish_ft_prob) ELSE (0.7 + 1.8 * d_m4_bearish_ft_prob) END AS F_Sustainability,
        CASE WHEN DBS_Score >= 0 THEN (1.0 - d_m5_bullish_reversal_risk) ELSE (1.0 - d_m5_bearish_reversal_risk) END AS F_Reversal 
    FROM Base_Score
),
DCS_Calculation AS (
    -- 6. Apply the final D.C.S. multiplicative formula
    SELECT trading_date, asset_id, DBS_Score, F_Commitment, F_Sustainability, F_Reversal,
        (DBS_Score * F_Commitment * F_Sustainability * F_Reversal) AS DCS_Final_Score
    FROM Factor_Calculation
)
-- 7. Final Insertion (omitted for brevity, use your full INSERT/UPDATE statement)
-- ...
