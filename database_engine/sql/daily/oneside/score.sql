...
Joined_Metrics AS (
    SELECT dc.trading_date, dc.asset_id,
        cm.D_M1_Bullish_Prob_LT, cm.D_M1_Bearish_Prob_LT, cm.D_M1_Alignment_Prob, 
...
Combined_D_M_Scores AS (
    -- 3a. Calculate D.M1 Score (Reverted to LT Bias * Alignment Multiplier)
    SELECT
        trading_date, asset_id,
        -- D.M1 Score: LT Bias provides a stable directional base, amplified by Alignment
        ((D_M1_Bullish_Prob_LT - D_M1_Bearish_Prob_LT) * (1.0 + D_M1_Alignment_Prob)) AS D_M1_Score,  
        -- 🔥 3b. D.M2 Score: PURE SHORT-TERM (ST) STRUCTURAL CONVICTION 🔥
        (
            -- Use ONLY the current 6-month structural probabilities (D_M2_Prob_ST)
...
Base_Score AS (
    ...
    SELECT
        trading_date, asset_id,
        -- D.M2 (Short-Term Momentum) is weighted higher (60%)
        (D_M1_Score * 0.40 + D_M2_Score * 0.60) AS DBS_Score, 
        d_m3_continuation_prob, d_m4_bullish_ft_prob, d_m4_bearish_ft_prob,
        d_m5_bullish_reversal_risk, d_m5_bearish_reversal_risk
...
DO UPDATE SET
    ...
    F_Reversal = EXCLUDED.DBS, DCS = EXCLUDED.DCS, DCS_Classification = EXCLUDED.DCS_Classification;