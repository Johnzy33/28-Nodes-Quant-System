...
pub fn render_dashboard(f: &mut Frame<'_>, state: &AppState, area: Rect) {
    let title = Span::styled(" 📜 CSV INGESTION COORDINATOR ", Style::default().fg(Color::LightGreen).bold());
    ...
    let chunks = Layout::default()
        ...
        .constraints([
            ...
            Constraint::Min(0),    // Bottom: Log history panel (takes the rest of the space)
        ])
        .split(inner_area);
    ...
    let refresh_msg = if state.global.is_loading {
        // When loading, use the real-time global status message for immediate feedback
        Line::from(vec![
            Span::styled("STATUS: ", Style::default().fg(Color::Yellow)),
            Span::raw(state.global.status_message.clone()), // Use the message sent from the worker
        ])
    } else {
        // When not loading, show the prompt to start
        Line::from(vec![
            Span::styled("STATUS: ", Style::default().fg(Color::Cyan)),
            Span::raw("Ready to process assets listed in "),
            Span::styled("ingestion_assets.json", Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)),
            Span::raw(". Press "),
            Span::styled("I", Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)),
            Span::raw(" to start ingestion loop."),
        ])
    };
    ...
    let log_items: Vec<ListItem> = state.csv_ingest.log_history
        .iter()
        .map(|m| {
            let style = if m.starts_with("STARTING:") {
                Style::default().fg(Color::LightCyan)
            } else if m.starts_with("✅ COMPLETED:") || m.starts_with("INFO:") {
                Style::default().fg(Color::Green)
            } else if m.starts_with("❌ FAILURE:") || m.starts_with("CRITICAL:") {
    ...
    let log_list = List::new(log_items)
        .block(Block::default().borders(Borders::TOP).title("Asset Status Log")) // Add a separating border/title
        // The last item in the list should always be visible (auto-scrolling)
