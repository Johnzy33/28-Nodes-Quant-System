-- Recreate the table definition (using your provided structure)
...
-- Initialize as a TimescaleDB Hypertable
...
WITH source_data_cast AS (
    -- NEW CTE: Explicitly cast source data before aggregation/joining (The previous fix)
    SELECT
        d.trading_date, d.asset_id, d.open::DOUBLE PRECISION AS open_c, 
        d.high, d.low, d.close::DOUBLE PRECISION AS close_c, 
        d.volume, d.bars, a.daily_atr
    FROM daily_views d
...
weekly_aggregate AS (
    ...
    SELECT
        DATE_TRUNC('week', sdc.trading_date) AS week_start,
        sdc.asset_id,
        (ARRAY_AGG(sdc.open_c ORDER BY sdc.trading_date ASC))[1] AS o_price,
        MAX(sdc.high) AS h_price,
        MIN(sdc.low) AS l_price,
        (ARRAY_AGG(sdc.close_c ORDER BY sdc.trading_date DESC))[1] AS c_price,
        SUM(sdc.volume) AS volume_sum,
        SUM(sdc.bars) AS bars_sum,
        MAX(sdc.high) - MIN(sdc.low) AS Current_Weekly_Range,
        AVG(sdc.daily_atr) AS Weekly_ATR 
    FROM source_data_cast sdc
    GROUP BY 1, 2
),
prior_week_data AS (
...
dow_to_index AS (
    -- 3. **FIXED CTE:** Only select necessary columns and enforce types
    SELECT 
        d.trading_date, 
        d.asset_id, 
        d.high::DOUBLE PRECISION AS high, 
        d.low::DOUBLE PRECISION AS low,   
...
break_metrics AS (
    ...
    SELECT
        ...
        -- We now need the Weekly Open from the WEEKLY_AGGREGATE, not from DOW_TO_INDEX
        (ARRAY_AGG(wa.o_price ORDER BY d.dow_index ASC))[1] AS Weekly_Open 
    FROM prior_week_data pwd
    JOIN dow_to_index d ON d.asset_id = pwd.asset_id AND DATE_TRUNC('week', d.trading_date) = pwd.week_start
    -- **CRITICAL CHANGE:** Join to weekly_aggregate to get the correct o_price for the week
    JOIN weekly_aggregate wa ON wa.asset_id = pwd.asset_id AND wa.week_start = pwd.week_start 
    WHERE pwd.Prior_Week_High IS NOT NULL OR pwd.Prior_Week_Low IS NOT NULL
    GROUP BY 1, 2
),
range_metrics AS (
    -- 6. W.M6 & W.M5 Final Range/Ratio Calculation
    SELECT
        b.week_start, b.asset_id, wa.Current_Weekly_Range, wa.Weekly_ATR, 
        b.Time_To_PWH_Break, b.Time_To_PWL_Break, pwd.Prior_Week_ATR,
        ...
        CASE WHEN b.Time_To_PWH_Break IS NOT NULL 
             THEN (wa.h_price - b.Weekly_Open) / NULLIF(pwd.Prior_Week_ATR, 0)
            WHEN b.Time_To_PWL_Break IS NOT NULL 
             THEN (b.Weekly_Open - wa.l_price) / NULLIF(pwd.Prior_Week_ATR, 0) 
             ELSE NULL 
        END AS Range_Used
...
classification_result AS (
    ...
    SELECT
        ...
        get_market_type(wa.o_price, wa.h_price, wa.l_price, wa.c_price) AS classification,
...
LEFT JOIN range_metrics r USING (week_start, asset_id)
